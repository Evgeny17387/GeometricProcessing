...
bool pushed = false;
std::string playButtonTitle = "Play";
...
int main(int argc, char *argv[])
{
	...
	menu.callback_draw_viewer_menu = [&]()
	{
		...
		ImGui::Combo("Optimization Strategy", (int *)(&optStrategy), "Gradient Descent\0Newton's Method\0\0");
		if (ImGui::Combo("Element Type", (int *)(&element_type), "Spring\0FEM Finite Differences\0FEM Analytic\0FEM AutoDiff\0\0"))
		{
			if (element_type == SPRING) {
				simMesh.createSpringsFromTriangles();
				simMesh.x = simMesh.X;
		...
		if (ImGui::Button(playButtonTitle.data()))
		{
			pushed = !pushed;

			if (pushed)
			{
				playButtonTitle = "Stop";
			}
			else
			{
		...
		ImGui::InputDouble("Max. Stress", &maxStress);

		ImGui::Combo("Mouse Mode", (int *)(&mouse_mode), "None\0Pin Node\0Apply Force\0\0");

		if (ImGui::Button("Apply Force Load"))
		{
